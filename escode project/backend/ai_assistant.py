...
class AIAssistant:
    ...
    def __init__(self):
        ...
        enabled_count = sum([self.use_bedrock, self.use_local_ai, self.use_external_free_ai])
        if enabled_count == 0:
            raise ValueError(
                "No AI provider enabled. Set USE_LOCAL_AI, USE_EXTERNAL_FREE_AI, or USE_BEDROCK to True"
            )
        if enabled_count > 1:
            raise ValueError("Multiple AI providers enabled. Only one can be active at a time")
    ...
    def _init_external_ai(self):
        ...
        if not self.api_key:
            raise ValueError("EXTERNAL_AI_API_KEY is required. Get free key from your AI provider")
    ...
    def _init_bedrock(self):
        ...
        raise NotImplementedError("Bedrock not configured yet. Set up AWS credentials and uncomment code above.")
    ...
    def generate_response(self, inquiry_subject, inquiry_message, context=None):
        ...
        if self.use_local_ai or self.use_external_free_ai:
            return self._generate_openai_compatible(inquiry_subject, inquiry_message, context)
        elif self.use_bedrock:
...
Generate only the email response text, no subject line."""
    ...
    def _generate_bedrock(self, subject, message, context):
        raise NotImplementedError("Bedrock implementation not active. Uncomment code above.")
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    "model": self.model,
                    "messages": [
                        {"role": "user", "content": f"Summarize this text in {max_length} characters or less:\n\n{text}"}
                    ],
                    "temperature": 0.5,
                    "max_tokens": 150
                },
        ...
        try:
            if self.use_local_ai or self.use_external_free_ai:
                ...
                response = requests.post(
                    f"{self.base_url}/chat/completions",
                    headers=headers,
                    json={"model": self.model, "messages": [{"role": "user", "content": "test"}], "max_tokens": 10},
                    timeout=10
                )
                response.raise_for_status()

                provider = "Ollama (Local)" if self.use_local_ai else "External Free AI"
                return {'success': True, 'message': f'{provider} connection successful', 'provider': provider, 'model': self.model}

            elif self.use_bedrock:
                return {'success': False, 'message': 'Bedrock not implemented yet', 'provider': 'bedrock'}

        except Exception as e:
            provider = "Ollama" if self.use_local_ai else "External AI" if self.use_external_free_ai else "Bedrock"
